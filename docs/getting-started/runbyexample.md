...
Please notice how above's cell is conveniently available in the environment via `$GIT_BRANCHES` (please inspect raw markdown). Please note that only cells named with uppercase letters and separated by underscores (min length is 3) are exported as environment variables:
